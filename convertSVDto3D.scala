...
package net.agittens.eofs.svdToNetCDF
...
object writeEOFs {
    ...
    def main(args: Array[String]) {
        ...
        val dims : Array[java.lang.Long] = Array(0,0)
        val maxdims : Array[java.lang.Long] = Array(0,0)
        ...
        var rawdata = Array.ofDim[double](dims(0), dims(1))
        H5.H5Dread(Udset_id, H5C.H5T_NATIVE_DOUBLE, memspace, filespace, daccess_id, rawdata)
        val brzUfloat = DenseMatrix(dims(1), dims(0), rawdata.flatMap(x=>x).map(_.toFloat)).t.copy // because breeze BDV are stored in col major order

        filespace = H5.H5Dget_space(Vdset_id)
        H5.H5Sget_simple_extent_dims(filespace, dims, maxdims)
        memspace = H5.H5Screate_simple(2, dims, null)
        rawdata = Array.ofDim[double](dims(0), dims(1))
        H5.H5Dread(Udset_id, H5C.H5T_NATIVE_DOUBLE, memspace, filespace, daccess_id, rawdata)
        val brzVfloat = DenseMatrix(dims(1), dims(0), rawdata.flatMap(x=>x).map(_.toFloat)).t.copy // because breeze BDV are stored in col major order

        filespace = H5.H5Dget_space(Sdset_id)
        memspace = H5.H5Screate_simple(1, Array[java.lang.Long](brzVfloat.numCols), null)
        rawdata = Array.ofDim[double](brzVfloat.numCols)
        H5.H5Dread(Sdset_id, H5C.H5T_NATIVE_DOUBLE, memspace, filespace, daccess_id, rawdata)
        val brzSfloat = DenseVector(rawdata.map(_.toFloat))
